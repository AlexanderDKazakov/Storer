...
class TestsStorer(unittest.TestCase):
    ...
    def test_long_loop_loading(self):
        ...
        Checking for not overloading internal data dict
        """
        s = Storer(path_dumps=PATH_DUMPS, dump_name=DUMP_NAME, verbose=True, compressed=False, separations=10000)
        cycles = 100_000
        for idx in range(cycles): 
            value = s.get(name=str(idx))
            if not value: s.put(what=idx, name=str(idx))
        ...
        s2 = Storer(path_dumps=PATH_DUMPS, dump_name=DUMP_NAME, verbose=True, compressed=False, separations=10000)
        s2.load()
        for idx in range(cycles): 
